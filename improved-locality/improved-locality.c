...
void setBorderConditions()
{
	for (i2 = 0; i2 < N2; ++i2)
	{
		for (i3 = 0; i3 < N3; ++i3)
		{
			...
			{
				a0[i2][i3][j] =
					exp(LAMBDA2 * i2 * H2 + LAMBDA3 * i3 * H3) *
					(j + (1. / 3.)) * TAU;
				a1[i2][i3][j] =
					exp(LAMBDA1 * (N1 - 1) * H1 + LAMBDA2 * i2 * H2 + LAMBDA3 * i3 * H3) *
					(j + (1. / 3.)) * TAU;
			}
		}
	}
	...
	for (i1 = 0; i1 < N1; ++i1)
	{
		for (i3 = 0; i3 < N3; ++i3)
		{
			...
			{
				b0[i1][i3][j] =
					exp(LAMBDA1 * i1 * H1 + LAMBDA3 * i3 * H3) *
					(j + (2. / 3.)) * TAU;
				b1[i1][i3][j] =
					exp(LAMBDA1 * i1 * H1 + LAMBDA2 * (N2 - 1) * H2 + LAMBDA3 * i3 * H3) *
					(j + (2. / 3.)) * TAU;
			}
		}
	}
	...
	for (i1 = 0; i1 < N1; ++i1)
	{
		for (i2 = 0; i2 < N2; ++i2)
		{
			...
			{
				c0[i1][i2][j] =
					exp(LAMBDA1 * i1 * H1 + LAMBDA2 * i2 * H2) *
					(j + 1) * TAU;
				c1[i1][i2][j] =
					exp(LAMBDA1 * i1 * H1 + LAMBDA2 * i2 * H2 + LAMBDA3 * (N3 - 1) * H3) *
					(j + 1) * TAU;
			}
		}
	}
	...
	for (int j = 0; j < T - 1; ++j)
	{
		...
		for (i1 = r1 * rank; i1 < r1 * (rank + 1); ++i1)
		{
			for (i3 = 0; i3 < N3; ++i3)
			{
				...
				{
					tempY[1][i1][i][i3] =
						alpha[i] * tempY[1][i1][i + 1][i3] + beta[i];
				}
			}
		}
		...
		if (rank)
		{
			MPI_Send(y[j + 1][rank * r1 + 1],
			         N2 * N3,
			         MPI_DOUBLE,
			         rank - 1,
			         0,
			         MPI_COMM_WORLD);
			MPI_Send(y[j + 1][rank * r1],
			         N2 * N3,
			         MPI_DOUBLE,
			         rank - 1,
			         0,
		...
		for (i1 = rank * r1; i1 < (rank + 1) * r1; ++i1)
		{
			for (i2 = 0; i2 < N2; ++i2)
			{
				for (i3 = 0; i3 < N3; ++i3)
				{
					double difference = fabs(calcExactSolution(TAU * j, H1 * i1, H2 * i2, H3 * i3) - y[j][i1][i2][i3]);
